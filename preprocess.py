...
import pickle
import jieba
jieba.load_userdict('data/raw/custom_dict.txt')  # 设置词库


def txt2csv():
    all_data=pd.read_csv('data/raw/all_docs.txt',sep='\001',header=None)
    all_data.columns=['id','title','doc']
    all_data.to_csv('data/raw/all_docs.csv',index=False)
    train_data=pd.read_csv('data/raw/train_docs_keywords.txt',sep='\t',header=None)
    ...
    for id,title,doc in tqdm(zip(ids,titles, docs)):
        ...
        for word, pos in word_tags:
            if word not in stop_words:
                if 1 < len(word) < 10:
                    new_word_tags.append((word, pos))
        new_doc = " ".join([word_tag[0] for word_tag in new_word_tags])
        # print(new_doc)
        # 保存分词好的数据
...
if __name__ == '__main__':
    # txt2csv()
    test_data_path = 'data/processed/test_doc.pkl'
    train_data_path = 'data/processed/train_docs.pkl'
    ...
    stop_words = open('data/raw/stop_words.txt', 'r', encoding='utf-8').read().split('\n')
    train_docs = generate_tokenized_doc(train_data_path, train_data, stop_words, allow_pos)
